    ...
    <div class="card text-white bg-info mb-3">
      ...
      <div class="card-body text-left ukuran-20">
        ...
        <form method="post" action="solusi.php" enctype="multipart/form-data" role="form">
          ...
          echo "<p>Jenis Anjing : " . $_SESSION['jenis'] . "</p>";
          echo "<p>Umur : " . $_SESSION['umur'] . " Bulan</p>";
          ...
          ?>
          <hr>
          <p>Gejala yang dialami anjing :</p>
          <?php
          include "fungsi.php";
          solusi($kode);
          ?>
          ...
          <hr>
          <?php
          $sql = "SELECT * from tb_solusi WHERE kode_penyakit='$kode'";
          $data = mysqli_query($connect, $sql);
          $row = mysqli_fetch_assoc($data);

          if ($row['kode_penyakit'] == "x-1" || $row['nama_penyakit'] == "x-2" || $row['nama_penyakit'] == "x-3" || $row['nama_penyakit'] == "x-4" || $row['nama_penyakit'] == "x-5") {
                ...
                <form action="solusi.php" method="post">
                  <div class="form-group">
                    ...
                    <select name="solusi" class="form-control" id="exampleFormControlSelect2">
                      ...
                      while ($row = mysqli_fetch_assoc($data)) {
                        if ($row['nama_penyakit'] != "x-1" && $row['nama_penyakit'] != "x-2" && $row['nama_penyakit'] != "x-3" && $row['nama_penyakit'] != "x-4" && $row['nama_penyakit'] != "x-5") {
                          echo '<option value="' . $row["nama_penyakit"] . '">' . $row["nama_penyakit"] . '</option>';
                        }
                      }
                      ?>
                    </select>
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" name="fakta" class="form-control" id="exampleFormControlInput1"
                      placeholder="contoh : Penurunan Berat Badan">
                  </div>
            ...
            $_SESSION['nama_penyakit'] = $row['nama_penyakit'];
            echo "<p>Penyakit yang dialami anjing adalah : <strong style='color:green'>" . $_SESSION['nama_penyakit'] . "</strong></p>";
        }
...
<script>
  ...
  if (!empty($_POST['masukan'])) {
    ...
    $sql2 = "INSERT INTO tb_kesimpulan (solusi, fakta, oleh, status) VALUES ('$solusi', '$fakta', '$oleh', '$status')";
